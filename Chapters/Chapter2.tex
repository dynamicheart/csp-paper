...
\begin{figure}
\centering
\includegraphics[scale=0.45]{Figures/memory_motivation1.png}
\decoRule
...
\begin{figure}
\centering
\includegraphics[scale=0.6]{Figures/memory_motivation2.png}
\decoRule
...
很长一段时间以来，数据中心都在使用着一体化服务器的架构，使得大多数内存分离技术都是基于这种架构去设计并实现的。
如分布式共享内存（distributed share memory）技术和远程换页（remote paging）技术。
那么，这些方法。

%LegoOS论文使用的是偏体系结构的技术划分办法，将内存分离技术分成两大类，一类是硬件辅助的内存分离技术；另一类是纯软件实现的内存分离技术。
%而INFINISWAP论文使用的是基于一体化服务器架构的技术划分方法，将内存分离技术分为远程内存换页技术（Remote Memory Paging）和分布式共享内存（Distributed Shared Memory）。
...
\begin{figure}
\centering
\includegraphics[scale=0.5]{Figures/infiniswap_architecture.png}
\decoRule
...
\begin{figure}
\centering
\includegraphics[scale=0.3]{Figures/infiniswap_slab_mapping.png}
\decoRule
...
\begin{figure}
\centering
\includegraphics[scale=0.3]{Figures/infiniswap_slab_eviction.png}
\decoRule
...
\begin{figure}
\centering
\includegraphics[scale=0.4]{Figures/infiniswap_evaluation1.png}
\decoRule
\caption{应用程序的性能}
...
\begin{figure}
\centering
\includegraphics[scale=0.3]{Figures/infiniswap_evaluation2.png}
\decoRule
\caption{集群的内存使用情况}
